...
const CartModal = () => {
    ...
    const { cart, removeItem, itemCount, isCartOpen, closeCart } = useCart();
    ...
    return (
        <div className={`${styles.overlay} ${isCartOpen ? styles.isOpen : ''}`} onClick={closeCart}>
            <div className={`${styles.modal} ${isCartOpen ? styles.isOpen : ''}`} onClick={(e) => e.stopPropagation()}>
                    ...
                    {(!cart || itemCount === 0) ? (
                        ...
                        cart.items.map(item => (
                            <div key={item.producto._id} className={styles.cartItem}>
                                ...
                                <Button variant="danger" onClick={() => removeItem(item.producto._id)}>Eliminar</Button>
                            </div>
                        ))
                    )}
                </div>
